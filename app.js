...
async function sendMessage(message) {
  ...
  if (data.providerError) notify("Live provider unreachable. Kaisen used Preview Core.");
  return data;
}
...
async function chatPage(token = renderToken) {
  ...
  page.innerHTML = `<div class="chat-layout reveal">
    <aside class="conversation-rail panel"><div class="conversation-head"><div><span class="panel-kicker">CONVERSATIONS</span><h2>Command history</h2></div><button id="newChat" aria-label="New conversation">＋</button></div><div class="conversation-list">${conversationCache.length ? conversationCache.map(item => `<button data-conversation="${item.id}" class="${item.id === activeConversationId ? "active" : ""}"><i>◇</i><span><strong>${escapeHtml(item.title)}</strong><small>${item.messages.length} SIGNALS · ${new Date(item.updatedAt).toLocaleDateString()}</small></span></button>`).join("") : `<p>NO SAVED CONVERSATIONS</p>`}</div><div class="provider-stack"><span class="panel-kicker">INTELLIGENCE ROUTER</span><div><i class="online"></i><span>AUTO</span><b>${providerConfig.providers.openai.configured || providerConfig.providers.claude.configured ? "KEYS CONFIGURED" : "PREVIEW FALLBACK"}</b></div><div><i class="${providerConfig.providers.openai.configured ? "online" : ""}"></i><span>OPENAI</span><b>${providerConfig.providers.openai.configured ? "KEY READY" : "NOT CONFIGURED"}</b></div><div><i class="${providerConfig.providers.claude.configured ? "online" : ""}"></i><span>CLAUDE</span><b>${providerConfig.providers.claude.configured ? "KEY READY" : "NOT CONFIGURED"}</b></div></div></aside>
